...
const styles = StyleSheet.create({
  ...
  inputField: { width: "100%", borderWidth: 1, borderColor: "#cbd5e1", borderRadius: 6, paddingHorizontal: 12, paddingVertical: 10, fontSize: 13, backgroundColor: "#ffffff", color: "#1e293b" },
  textAreaField: { minHeight: 60, textAlignVertical: "top" },
  pickerContainer: { width: "100%", borderWidth: 1, borderColor: "#cbd5e1", borderRadius: 6, backgroundColor: "#ffffff", overflow: "hidden", height: 48, justifyContent: 'center' },
  ...
  radioOuter: { width: 16, height: 16, borderRadius: 8, borderWidth: 1, alignItems: "center", justifyContent: "center", marginRight: 6 },
  ...
  btnExport: { flexDirection: "row", alignItems: "center", justifyContent: "center", backgroundColor: "#4f46e5", paddingHorizontal: 12, paddingVertical: 8, borderRadius: 6, elevation: 1 },
  btnSave: { flexDirection: "row", alignItems: "center", justifyContent: "center", backgroundColor: "#16a34a", paddingHorizontal: 12, paddingVertical: 8, borderRadius: 6, elevation: 1 },
...
const InputGroup = ({ label, name, value, onChange, keyboardType = "default", placeholder = "", width = "100%" }: any) => (
...
const SelectGroup = ({ label, name, value, onChange, options, width = "100%" }: any) => (
  <View style={[styles.inputContainer, { width }]}>
    <Text style={styles.inputLabel} numberOfLines={1}>{label}</Text>
    <View style={styles.pickerContainer}>
      <Picker selectedValue={value || ""} onValueChange={(itemValue) => onChange(name, itemValue)} style={{ height: 48 }}>
        <Picker.Item label="Seçiniz..." value="" color="#94a3b8" />
        {options.map((opt: string) => <Picker.Item key={opt} label={opt} value={opt} />)}
      </Picker>
    </View>
  </View>
);

const TextAreaGroup = ({ label, name, value, onChange, rows = 2, width = "100%" }: any) => (
...
const RadioGroup = ({ label, name, value, options, onChange, width = "100%" }: any) => (
  <View style={[styles.inputContainer, { width, marginTop: 4 }]}>
...
export default function HastaAnamnezMiniApp() {
  ...
  // --- NATIVE PDF EXPORT WITH EXPO-PRINT ---
  const exportPDF = async () => {
    ...
    let htmlContent = `
      ...
      <head>
        ...
        <style>
          body { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; padding: 20px; color: #111; font-size: 11px; }
          .header { border-bottom: 3px solid #bedcff; padding-bottom: 10px; margin-bottom: 20px; }
          .univ-title { font-size: 12px; color: #555; }
          .patient-info { font-weight: bold; font-size: 13px; margin-top: 5px; }
          .footer { margin-top: 30px; font-weight: bold; font-size: 11px; display: flex; justify-content: space-between; color: #333; }
    ...
    filesToExport.forEach((f, index) => {
      ...
      htmlContent += `
        <div class="header">
           ...
           <div class="patient-info">Dosya No : ${val("kimlik_dosyaNo") || "-"} &nbsp;|&nbsp; Hasta Adi Soyadi : ${val("kimlik_adSoyad").toUpperCase()}</div>
        </div>
        <div class="row" style="margin-bottom: 20px;">
          <div class="col-left">00:00<br/>Klinik Arsiv</div>
          <div class="col-right" style="font-weight: bold; font-size: 14px;">${formTypeStr}<br/>Muayene</div>
        </div>
      `;
    ...
    try {
      // expo-print kütüphanesi kullanarak HTML'i anında PDF'e çeviriyoruz
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={{ flex: 1 }}>
        ...
        <ScrollView style={{ flex: 1, padding: 8 }} contentContainerStyle={{ paddingBottom: 40 }}>
          <View style={styles.card}>
            ...
            <View style={styles.innerCard}>
              ...
              <View style={styles.row}>
                ...
                <InputGroup label="Görüşme (YYYY-AA-GG)" name="kimlik_gorusmeTarihi" value={formData.kimlik_gorusmeTarihi} onChange={handleInputChange} width="50%" />
                <InputGroup label="Doğum (YYYY-AA-GG)" name="kimlik_dogumTarihi" value={formData.kimlik_dogumTarihi} onChange={handleInputChange} width="50%" />
          ...
          <View style={styles.modalContent}>
            ...
            {exportRange.includeAll && (
              <View style={{ backgroundColor: "#f8fafc", padding: 12, borderRadius: 8, borderWidth: 1, borderColor: "#e2e8f0", marginBottom: 20 }}>
                ...
                <View style={{ gap: 12 }}>
                  <View>
                    <Text style={{ fontSize: 11, fontWeight: "bold", color: "#475569", marginBottom: 4 }}>Başlangıç (YYYY-AA-GG)</Text>
                    <TextInput style={styles.inputField} onChangeText={(text) => setExportRange({ ...exportRange, start: text })} placeholder="2024-01-01" placeholderTextColor="#94a3b8" />
                  </View>
                  <View>
                    <Text style={{ fontSize: 11, fontWeight: "bold", color: "#475569", marginBottom: 4 }}>Bitiş (YYYY-AA-GG)</Text>
                    <TextInput style={styles.inputField} onChangeText={(text) => setExportRange({ ...exportRange, end: text })} placeholder="2024-12-31" placeholderTextColor="#94a3b8" />
                  </View>
                </View>
              </View>
            )}
